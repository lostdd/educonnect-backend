...
from app.auth.pydantic_models import Login, UserBase, UserAddDB, UserReg, UserAuth, UserActivate, UserResetPassword
...
async def register_user(db_session: TransactionSessionDep,
    ...
    if app_settings.PASSWORD_CHECK and not app.auth.utils.password_check(reg_form.password, app_settings.PASSWORD_MIN_LENGTH)['password_ok']:
...
async def activate_user(db_session: TransactionSessionDep,
    ...
    user = await UserDAO.find_one_or_none(db_session, Login(login=login))
    if user:
        if user.reset_token == activation_form.reset_token:
            match user.completed_registration, user.disabled:
                case False, False:
                    ...
                    await UserDAO.update_one_by_id(db_session, user.id, ValuesModel(completed_registration=True))
        ...
        await UserDAO.update_one_by_id(db_session, user.id, ValuesModel(reset_token=token_urlsafe(128)))
        ...
        if user.reset_token == reset_form.reset_token:
            match user.completed_registration, user.disabled:
                case True, False:
                    ...
                    if app_settings.PASSWORD_CHECK and not app.auth.utils.password_check(reset_form.password, app_settings.PASSWORD_MIN_LENGTH)['password_ok']:
                        raise api_exceptions.PasswordIsNotOK
                    await UserDAO.update_one_by_id(db_session, user.id, ValuesModel(password=app.auth.utils.get_password_hash(reset_form.password)))
...
async def login(response: Response, auth_form: UserAuth, db_session: TransactionSessionDep):
    ...
    # print(app.auth.utils.get_password_hash(auth_form.password), user.password)
